...
class ConcatModel(BaseModel):
  ...
  def forward(self, ped_crops, masks, bbox=None, act=None, pose=None, obj_bbox=None, obj_cls=None):
    B, T, _,_,_ = ped_crops.shape

    if self.branch in ['both', 'ped']:
      # ped_crops: (bt, 30, 3, 224, 224)
      ped_crops = ped_crops.type(self.dtype).view(-1, 3, 224, 224)
    ...
    if os.path.exists(self.extract_feats_dir):
  ...
  def extract_feats(self, ped_crops, masks, idx):
    """
    ped_crops: (1, 3, 224, 224)
    masks: list of len 1; each item being a dict w/ key 1-4
    """
    assert(ped_crops.dim()==4), print('extract_feats does not support batch mode. Data is obtained directly from __getitem__.')
    assert(len(masks) == 1)
    T = ped_crops.shape[0]
    ...
    # ped_feats: shape: (T, D (e.g.512), 1, 1)
    # ped_feats = self.ped_encoder(ped_crops.to(self.device)).cpu()
    ped_feats = self.ped_encoder(ped_crops.type(self.dtype)).cpu()
    ped_feats = ped_feats.view(T, -1)
